...
# Suppress standard Python warnings from flooding the terminal
...
class Hyperparameters:
    ...
    # Enforce frequent logging default to 50
...
class BitLinear(nn.Linear):
    def forward(self, x: Tensor) -> Tensor:
        ...
        return F.linear(x_quant, w_quant.to(x.dtype), bias)
...
def main() -> None:
    ...
    if master_process:
        ...
        # PREVENT KONSOLE RAM BLOWOUT
        # Redirect all hidden PyTorch/ROCm error/warning spam directly to a file
        errfile = f"logs/{args.run_id}_errors.txt"
        sys.stderr = open(errfile, "w")
        print(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Log created at {logfile}")

    def log0(msg: str, console: bool = True) -> None:
        if not master_process:
            return
        # Add beautiful timestamps to all lines
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        formatted_msg = f"[{timestamp}] {msg}"
        if console:
            print(formatted_msg, flush=True) # Force flush to terminal immediately
    ...
    try:
        ...
        gpu_info = "nvidia-smi not found (Running locally on AMD)"
    ...
    if not args.tokenizer_path.endswith(".model"): raise ValueError("Must be sentencepiece model")
    ...
    for module in base_model.modules():
        if isinstance(module, CastedLinear) or isinstance(module, BitLinear):
            module.float()
    ...
    log0(f"model_params:{n_params} (BitNet 1.58-bit implementation enabled)")
    ...
    # NEW: Initialize the GradScaler to protect against FP16 Overflow
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                ...
                # Use scaler for warmup pass to prevent early NaNs
                scaler.scale(warmup_loss * grad_scale).backward()
            
            # Unscale gradients for clipping
            for opt in optimizers:
                scaler.unscale_(opt)
            
            clip_val = args.grad_clip_norm if args.grad_clip_norm > 0 else 1.0
            torch.nn.utils.clip_grad_norm_(base_model.parameters(), clip_val)
            
            # Step optimizers using scaler
            for opt in optimizers:
                scaler.step(opt)
    ...
    while True:
        ...
        should_log_train = args.train_log_every > 0 and (step % args.train_log_every == 0 or last_step)
    ...
    # -----------------------------
    if master_process:
        log0("Snapping BitNet weights to ternary values to maximize zlib compression...")
    with torch.no_grad():
        for module in base_model.modules():
            if isinstance(module, BitLinear):
                module.weight.copy_(weight_quant(module.weight))
    ...
    if master_process:
        ...
        log0(f"Serialized model int8+zlib: {quant_file_bytes} bytes (payload ratio:{ratio:.2f}x)")
